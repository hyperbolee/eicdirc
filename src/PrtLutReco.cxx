...
void PrtLutReco::Run(Int_t start, Int_t end){
	...
	int ievent(0), tMCP(0), tPix(0), tChan(0);
	double tX(0), tY(0), expTime(0), diffTime(0);
	...
	lTree->Branch("PID",&tofPid,"tofPid/I"); // particle ID
	...
	lTree->Branch("theta",&tangle,"tangle/D"); // theta_C
	//lTree->Branch("phi",&tPhi,"tPhi/D"); // azimuth angle
	lTree->Branch("time",&hitTime,"hitTime/D"); // arrival time of hit
	lTree->Branch("expt",&expTime,"expTime/D"); // expected arrival time
	lTree->Branch("diff",&diffTime,"diffTime/D"); // difference in times

	timeRes = 2; // ns PrtManager::Instance()->GetTimeRes();
	...
	Int_t ntotal=0;
	for (ievent=0; ievent<end; ievent++){
		...
		for(Int_t h=0; h<nHits; h++) {
			...
			for(int i=0; i<size; i++){
				...
				for(int u=0; u<4; u++){
					...
					expTime  = bartime + evtime;
					diffTime = hitTime-expTime;
					//std::cout << Form("bartime: %.4f\nevtime: %.4f\nhittime: %.4f\ntimediff: %.4f\n",bartime,evtime,hitTime,timediff) << std::endl;
